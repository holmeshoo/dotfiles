...
### 免責事項
- **注意**: 本プロジェクトは現在も開発・調整中であり、予期せぬバグや動作不良が含まれている可能性があります。完全に安定したものではないことをご理解の上、ご利用ください。
...
- **macOS システム設定の注意**: macOS 環境で実行する場合、`macos/defaults.sh` によってシステム設定（Dock, Finder, キーボード等）が強制的に上書きされます。適用される内容を事前に確認し、必要に応じて編集してから実行してください。
